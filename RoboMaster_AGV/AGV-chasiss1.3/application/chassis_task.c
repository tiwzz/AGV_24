...
static void chassis_init(chassis_move_t *chassis_move_init)
{
	...
	// 斜坡函数初始化
	 ramp_init(&chassis_move_init->vx_ramp, CHASSIS_RAMP_X_NUM, MAX_RAMP_X_OUT, MIN_RAMP_X_OUT);
   ramp_init(&chassis_move_init->vy_ramp, CHASSIS_RAMP_Y_NUM, MAX_RAMP_Y_OUT, MIN_RAMP_Y_OUT);
	// 轮电机转动方向初始化
...
//	fp32 constant_3508 = 4.081f;  //3508电机的机械损耗
	...
	input_power = max_power_limit - chassis_motor->buffer_pid.out; //通过裁判系统的最大功率//为啥是减号：因为pid是负反馈，即设定值减当前值
	...
	if (get_cap.capvot > 15) //当超电电压大于某个值(防止C620掉电)//手册极限为12V
	{
		...
		{
			chassis_motor->power_control.POWER_MAX = 180;		
		}
		else
		{
...
//	chassis_motor->power_control.forecast_total_power = LIMIT_MAX_MIN(chassis_motor->power_control.forecast_total_power,300,-300);
